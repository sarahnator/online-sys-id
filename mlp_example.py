...
def qualitative_evaluation():
    ...
    for i in range(n):
        ...
        mlp_state = mlp_traj[-1]
        # expand _dt to match the batch size and number of points
        expanded_dt = _dt.expand(mlp_state.shape[0], -1).expand(-1, mlp_state.shape[1])
        mlp_dstate = model.forward(mlp_state)
        mlp_prediction = mlp_state + mlp_dstate
        mlp_traj.append(mlp_prediction)


    ground_truth_traj = torch.stack(ground_truth_traj, dim=0).detach().cpu().numpy()
    mlp_traj = torch.stack(mlp_traj, dim=0).detach().cpu().numpy()
    ...
    for i in range(3):
        for j in range(3):
            ...
            (_m,) = ax[i, j].plot(mlp_traj[:, traj_idx, 0, 0], mlp_traj[:, traj_idx, 0, 1], color='orange', label='MLP')
...
def quantitative_evaluation(mu_function: Callable = mu_piecewise_constant, trange: int = 5000):
    ...
    losses_mlp = []  # to store the losses for each step
    losses_mlp_cumulative = []  # to store the losses of the cumulative version of maml for each step
    losses_mlp_window  = []  # to store the losses of the windowed version of maml for each step
    ...
    with tqdm.trange(trange) as tqdm_bar:
        for step in tqdm_bar:
            ...
            # Compute mlp predictions with update on the single observation
            mlp_pred = model.forward(_y0, adapted_params)
            loss = model.loss_function(mlp_pred, _y1)

            # # Compute mlp predictions with cumulative update
            # cumulative_mlp_pred = model.forward(_y0, cumulative_adapted_params)
            # cumulative_loss = model.loss_function(cumulative_mlp_pred, _y1)

            # Compute mlp predictions with windowed update
            windowed_mlp_pred = model.forward(_y0, windowed_adapted_params)
            windowed_loss = model.loss_function(windowed_mlp_pred, _y1)
            
            # Store the losses
            losses_mlp.append(loss.item())
            # losses_mlp_cumulative.append(cumulative_loss.item())
            losses_mlp_window.append(windowed_loss.item())
    ...
    ax.plot(losses_mlp, label="MAML NODE Loss", color='blue')
    # ax.plot(losses_mlp_cumulative, label="MAML (cumulative data) NODE Loss", color='orange')
    ax.plot(losses_mlp_window, label="MAML (windowed data) NODE Loss", color='green')
    plt.legend()
    plt.tight_layout()
    # plt.show()
    ...
    losses_mlp_cumulative = torch.tensor(losses_mlp_cumulative, device=device)
    torch.save({
...
if __name__ == "__main__":
    ...
    quantitative_evaluation(mu_function=mu_piecewise_constant, trange=2500)
    quantitative_evaluation(mu_function=mu_sinusoidal_modulation, trange=2500)
    quantitative_evaluation(mu_function=mu_linear_ramp, trange=2500)
    quantitative_evaluation(mu_function=mu_constant, trange=2500)
