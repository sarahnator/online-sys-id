...
def least_squares(
    ...
    Args:
        f (torch.Tensor): Function evaluations [batch_size, n_points, n_features]   (observations/measurements)
        g (torch.Tensor): Basis functions evaluations [batch_size, n_points, n_features, n_basis] (regressor matrix)
        inner_product (callable): Inner product function
...
def _rls_woodbury(
    ...
    # Compute residual: note that this assumes the function encoder model does not have a residual network
